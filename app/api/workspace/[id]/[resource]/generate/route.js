...
12-16 entries (3-4/week). All dates must be after today. Start first Monday after today.`,
...
export async function POST(request, { params }) {
  try {
    ...
    if (resource !== 'leads' && !PROMPTS[resource]) return NextResponse.json({ error: 'Invalid resource' }, { status: 400 });
    ...
    if (resource === 'leads') {
      items = await searchLeads(company, ws);
    } else {
